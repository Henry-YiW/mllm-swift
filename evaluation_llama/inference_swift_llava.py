...
def swift_forward(input_ids, model, tokenizer, max_new_tokens, statistics=None, optimizer=None, utility=None,
                  logits_processor=None, max_steps=512, pixel_values=None):
...
if __name__ == "__main__":
    ...
    if args.cache_hit:
        ...
        _attn_skip_layer_id_set = np.arange(1, model.language_model.config.num_hidden_layers - 1, 2)  # keep the first and last layer
        _mlp_skip_layer_id_set = np.arange(1, model.language_model.config.num_hidden_layers - 1, 2)

    model.set_skip_layers(_attn_skip_layer_id_set, _mlp_skip_layer_id_set)

    # Bayes Optimization Settings
    pbounds = {f"x{i}": (0, 1) for i in range((model.language_model.config.num_hidden_layers - 2) * 2)} # keep the first and last layer
