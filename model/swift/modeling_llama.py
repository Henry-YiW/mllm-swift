...
class LlamaRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, seq_len=None):
        ...
        # print("SFJSLKDFJDSLJF 1", x.shape, seq_len, self.max_seq_len_cached)
        # print("SFJSLKDFJDSLJF 2", self.cos_cached.shape, self.sin_cached.shape)
    ...
    def forward(
        ...
        if self.training:
            ...
            if enabled_draft and self.layer_id in _attn_skip_layer_id_set:
                ...
                # Self Attention
                #print("type of past_key_value", type(past_key_value[0]))
                hidden_states, self_attn_weights, present_key_value = self.self_attn(
                    hidden_states=hidden_states,
                    attention_mask=attention_mask,
                    position_ids=position_ids,
    ...
    def forward(
        ...
        if past_key_values is not None:
            for past_key_value in past_key_values:
                if past_key_value is not None:
                    # print('full past_key_value', past_key_value)
                    # print('past_key_value', past_key_value[0])
