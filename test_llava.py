...
from model.swift.kv_cache import initialize_past_key_values  # KVCache definitions are here
...
def main():
    ...
    # We assign the list of KVCache objects to the model.
    model.past_key_values = past_key_values
    # Optionally, keep references to the raw data and length tracker.
    ...
    for layer_idx, kv_pair in enumerate(new_past):
        for j in range(2):  # 0 for key, 1 for value.
            # Instead of appending the full history, only get the last tokens.
            incremental = new_past[layer_idx][j][..., -1:, :]  # Extract last token representations.
    ...
    for _ in range(max_new_tokens):
        ...
        outputs = model(
            ...
            past_key_values=model.past_key_values,  # Use our KVCache objects.
            use_cache=True,
            return_dict=True
        )
