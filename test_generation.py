...
def main():
    ...
    model_correct = LlavaForConditionalGeneration.from_pretrained(
        model_id,
        low_cpu_mem_usage=True,
        device_map={'': 'cuda:0'}  # Force the entire model onto cuda:0
    ).to("cuda:0")
    ...
    past_key_values, past_key_values_data, current_length_data = initialize_past_key_values(model_correct.language_model)
    ...
    #test auto_regressive generation with kv cache by running forward pass
    past_key_values = outputs_correct.past_key_values
    #print the length of the past key values
    print("past_key_values", past_key_values[-1][0].shape)
    #print the shape of the input_ids
    print("input_ids shape after forward pass", input_ids.shape)
    max_new_tokens = 100
    generated_ids = input_ids.clone()  # Start with the initial input IDs
    # last_token = torch.argmax(outputs_correct.logits[:, -1, :], dim=-1, keepdim=True)
    # generated_ids = torch.cat([generated_ids, last_token], dim=1)
    #decode the generated ids
    generated_ids_decoded = tokenizer.decode(generated_ids[0][-1], skip_special_tokens=True)
    print("-----------")
    print("generated_ids_decoded", generated_ids_decoded)
    print("-----------")
    
    with torch.inference_mode():
        for i in range(max_new_tokens):
            # Pass only the last generated token and the past key values
            outputs_correct = model_correct(
                input_ids=generated_ids[:, -1:],  # Only the last token
                pixel_values=None,  # Include pixel values if needed
                past_key_values=past_key_values
            )
            # Get the logits for the last token and select the next token
            next_token = torch.argmax(outputs_correct.logits[:, -1, :], dim=-1, keepdim=True)
    ...
    generated_tokens_decoded = tokenizer.decode(torch.argmax(generated_ids, dim=-1))
    #print("generated_tokens_decoded", generated_tokens_decoded)
    #print the text of the generated tokens
    print("generated_text", tokenizer.decode(generated_ids[0], skip_special_tokens=True))
